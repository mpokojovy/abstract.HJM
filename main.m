...
%% Estimation of Merton's long rate model
[mu_hat, sigma_hat] = Merton_inv_map(T_grid, yield_obs(:, 1));

%% Inverse problem for the abstract Heath-Jarrow-Morton model
sigma_HJM_hat = HJM_inv_map(T_grid, X_grid_ext, Y_obs, 0.99);

n_mode = size(sigma_HJM_hat, 1);
...
axis([min(X_grid_ext) max(X_grid_ext) min(sigma_HJM_hat, [], 'all') max(sigma_HJM_hat, [], 'all')]);

for i = 1:n_mode
    plot(X_grid_ext, sigma_HJM_hat(i, :), 'LineWidth', 2, 'MarkerSize', 0.01);
end
...
% Predicting the short rates f_{t}(0) with Merton's model
pred0 = Merton_fwd_map(T_grid_pred, yield_obs(end, 1), mu_hat, sigma_hat, n_rep);
% Predicting integrated forward rates with HJM model
pred  = HJM_fwd_map(T_grid_pred, X_grid_ext, Y_obs(end, :), pred0, sigma_HJM_hat, n_rep);
...
for i = 1:length(X_grid_ext)
    yield_lq(i)  = quantile(yield_pred(i, :), conf/2);
    yield_hq(i)  = quantile(yield_pred(i, :), 1 - conf/2);
...
legend({'Observed yield curve', 
       ['Lower $', num2str(100*(1 - conf)), '\%$ pointwise prediction bound'],
       ['Upper $', num2str(100*(1 - conf)), '\%$ pointwise prediction bound'],
